...
\@x{}\moduleLeftDash\@xx{ {\MODULE} message\_queue3}\moduleRightDash\@xx{}%
\@x{ {\EXTENDS} TLC ,\, Integers ,\, Sequences}%
\@x{ {\CONSTANTS} MaxQueueSize}%
\@pvspace{8.0pt}%
\pcalsymbolstrue
\pcalshadingtrue
\csyntaxfalse
\@x{\@s{8.2} {\p@mmalgorithm} message\_queue}%
\@pvspace{8.0pt}%
\@x{ {\p@variable} queue \.{=} {\langle} {\rangle} {\p@semicolon}}%
\@pvspace{8.0pt}%
\@x{ {\p@define}}%
\@x{\@s{16.4} BoundedQueue \.{\defeq} Len ( queue ) \.{\leq} MaxQueueSize}%
\@x{ {\p@end} {\p@define} {\p@semicolon}}%
\@pvspace{8.0pt}%
\@x{ {\p@macro} add\_to\_queue ( val ) {\p@begin}}%
\@x{\@s{16.4} {\p@await} Len ( queue ) \.{<} MaxQueueSize {\p@semicolon}}%
\@x{\@s{16.4} queue \.{:=} Append ( queue ,\, val ) {\p@semicolon}}%
\@x{ {\p@end} {\p@macro} {\p@semicolon}}%
\@pvspace{8.0pt}%
\@x{ {\p@procedure} add\_to\_queue ( val \.{=}\@w{} ) {\p@begin}}%
\@x{\@s{16.4} Add\@s{.5}\textrm{:}\@s{3}}%
\@x{\@s{32.8} {\p@await} Len ( queue ) \.{<} MaxQueueSize {\p@semicolon}}%
\@x{\@s{32.8} queue \.{:=} Append ( queue ,\, val ) {\p@semicolon}}%
\@x{\@s{32.8} {\p@return} {\p@semicolon}}%
\@x{ {\p@end} {\p@procedure} {\p@semicolon}}%
\@pvspace{8.0pt}%
\@x{ {\p@process} writer \.{=}\@w{writer}}%
\@x{ {\p@begin} Write\@s{.5}\textrm{:}\@s{3}}%
\@x{\@s{16.4} {\p@while} {\TRUE} {\p@do}}%
\@x{\@s{32.8} {\p@call} add\_to\_queue (\@w{msg} )}%
\@x{\@s{16.4} {\p@end} {\p@while} {\p@semicolon}}%
\@x{ {\p@end} {\p@process} {\p@semicolon}}%
\@pvspace{8.0pt}%
\@x{ {\p@process} reader \.{\in} \{\@w{r1} ,\,\@w{r2} \}}%
\@x{ {\p@variables} current\_message \.{=}\@w{none} {\p@semicolon}}%
\@x{ {\p@begin} Read\@s{.5}\textrm{:}\@s{3}}%
\@x{\@s{16.4} {\p@while} {\TRUE} {\p@do}}%
\@x{\@s{32.8} {\p@await} queue \.{\neq} {\langle} {\rangle} {\p@semicolon}}%
\@x{\@s{32.8} current\_message \.{:=} Head ( queue ) {\p@semicolon}}%
\@x{\@s{32.8} queue \.{:=} Tail ( queue ) {\p@semicolon}}%
\@x{\@s{32.8} {\p@either}}%
\@x{\@s{49.19} {\p@skip} {\p@semicolon}}%
\@x{\@s{32.8} {\p@or}}%
\@x{\@s{49.19} NotifyFailure\@s{.5}\textrm{:}\@s{3}}%
\@x{\@s{65.6} current\_message \.{:=}\@w{none} {\p@semicolon}}%
\@x{\@s{65.6} {\p@call} add\_to\_queue ( self ) {\p@semicolon}}%
\@x{\@s{32.8} {\p@end} {\p@either} {\p@semicolon}}%
\@x{\@s{16.4} {\p@end} {\p@while} {\p@semicolon}}%
\@x{ {\p@end} {\p@process} {\p@semicolon}}%
\@pvspace{8.0pt}%
\@x{ {\p@end} {\p@algorithm}}%
...
\@x{}%
\@y{\@s{0}%
 BEGIN TRANSLATION (\ensuremath{chksum(pcal) \.{=}} ``\ensuremath{cc8acc17}''
 \ensuremath{\.{\land} chksum(tla) \.{=}} ``\ensuremath{887d62be}'')
}%
\@xx{}%
\@x{ {\VARIABLES} queue ,\, pc ,\, stack}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 define statement
...
\@xx{}%
\@x{ BoundedQueue \.{\defeq} Len ( queue ) \.{\leq} MaxQueueSize}%
\@pvspace{8.0pt}%
\@x{ {\VARIABLES} val ,\, current\_message}%
\@pvspace{8.0pt}%
 \@x{ vars \.{\defeq} {\langle} queue ,\, pc ,\, stack ,\, val ,\,
 current\_message {\rangle}}%
\@pvspace{8.0pt}%
 \@x{ ProcSet \.{\defeq} \{\@w{writer} \} \.{\cup} ( \{\@w{r1} ,\,\@w{r2} \}
 )}%
\@pvspace{8.0pt}%
\@x{ Init \.{\defeq}}%
\@y{\@s{0}%
...
\@xx{}%
\@x{\@s{35.70} \.{\land} queue \.{=} {\langle} {\rangle}}%
\@x{\@s{35.70}}%
\@y{\@s{0}%
 Procedure \ensuremath{add\_to\_queue
}}%
\@xx{}%
\@x{\@s{35.70} \.{\land} val \.{=} [ self \.{\in} ProcSet \.{\mapsto}\@w{} ]}%
\@x{\@s{35.70}}%
\@y{\@s{0}%
 Process reader
}%
\@xx{}%
 \@x{\@s{35.70} \.{\land} current\_message \.{=} [ self \.{\in} \{\@w{r1}
 ,\,\@w{r2} \} \.{\mapsto}\@w{none} ]}%
 \@x{\@s{35.70} \.{\land} stack \.{=} [ self \.{\in} ProcSet \.{\mapsto}
 {\langle} {\rangle} ]}%
 \@x{\@s{35.70} \.{\land} pc \.{=} [ self \.{\in} ProcSet \.{\mapsto} {\CASE}
 self \.{=}\@w{writer} \.{\rightarrow}\@w{Write}}%
 \@x{\@s{160.39} {\Box}\@s{10.30} self \.{\in} \{\@w{r1} ,\,\@w{r2} \}
 \.{\rightarrow}\@w{Read} ]}%
\@pvspace{8.0pt}%
\@x{ Add ( self ) \.{\defeq} \.{\land} pc [ self ] \.{=}\@w{Add}}%
\@x{\@s{61.27} \.{\land} Len ( queue ) \.{<} MaxQueueSize}%
\@x{\@s{61.27} \.{\land} queue \.{'} \.{=} Append ( queue ,\, val [ self ] )}%
 \@x{\@s{61.27} \.{\land} pc \.{'} \.{=} [ pc {\EXCEPT} {\bang} [ self ] \.{=}
 Head ( stack [ self ] ) . pc ]}%
 \@x{\@s{61.27} \.{\land} val \.{'} \.{=} [ val {\EXCEPT} {\bang} [ self ]
 \.{=} Head ( stack [ self ] ) . val ]}%
 \@x{\@s{61.27} \.{\land} stack \.{'} \.{=} [ stack {\EXCEPT} {\bang} [ self ]
 \.{=} Tail ( stack [ self ] ) ]}%
\@x{\@s{61.27} \.{\land} {\UNCHANGED} current\_message}%
\@pvspace{8.0pt}%
\@x{ add\_to\_queue ( self ) \.{\defeq} Add ( self )}%
\@pvspace{8.0pt}%
\@x{ Write \.{\defeq} \.{\land} pc [\@w{writer} ] \.{=}\@w{Write}}%
 \@x{\@s{44.83} \.{\land} \.{\land} stack \.{'} \.{=} [ stack {\EXCEPT}
 {\bang} [\@w{writer} ] \.{=} {\langle} [ procedure
 \.{\mapsto}\@s{4.1}\@w{add\_to\_queue} ,\,}%
\@x{\@s{237.82} pc\@s{31.36} \.{\mapsto}\@s{4.10}\@w{Write} ,\,}%
 \@x{\@s{237.82} val\@s{27.82} \.{\mapsto}\@s{4.1} val [\@w{writer} ] ]
 {\rangle}}%
\@x{\@s{235.26} \.{\circ} stack [\@w{writer} ] ]}%
 \@x{\@s{55.94} \.{\land} val \.{'} \.{=} [ val {\EXCEPT} {\bang} [\@w{writer}
 ] \.{=}\@w{msg} ]}%
 \@x{\@s{44.83} \.{\land} pc \.{'} \.{=} [ pc {\EXCEPT} {\bang} [\@w{writer} ]
 \.{=}\@w{Add} ]}%
 \@x{\@s{44.83} \.{\land} {\UNCHANGED} {\langle} queue ,\, current\_message
 {\rangle}}%
\@pvspace{8.0pt}%
\@x{ writer \.{\defeq} Write}%
\@pvspace{8.0pt}%
\@x{ Read ( self ) \.{\defeq} \.{\land} pc [ self ] \.{=}\@w{Read}}%
\@x{\@s{65.22} \.{\land} queue \.{\neq} {\langle} {\rangle}}%
 \@x{\@s{65.22} \.{\land} current\_message \.{'} \.{=} [ current\_message
 {\EXCEPT} {\bang} [ self ] \.{=} Head ( queue ) ]}%
\@x{\@s{65.22} \.{\land} queue \.{'} \.{=} Tail ( queue )}%
\@x{\@s{65.22} \.{\land} \.{\lor} \.{\land} {\TRUE}}%
 \@x{\@s{87.44} \.{\land} pc \.{'} \.{=} [ pc {\EXCEPT} {\bang} [ self ]
 \.{=}\@w{Read} ]}%
 \@x{\@s{76.33} \.{\lor} \.{\land} pc \.{'} \.{=} [ pc {\EXCEPT} {\bang} [
 self ] \.{=}\@w{NotifyFailure} ]}%
\@x{\@s{65.22} \.{\land} {\UNCHANGED} {\langle} stack ,\, val {\rangle}}%
\@pvspace{8.0pt}%
 \@x{ NotifyFailure ( self ) \.{\defeq} \.{\land} pc [ self ]
 \.{=}\@w{NotifyFailure}}%
 \@x{\@s{100.87} \.{\land} current\_message \.{'} \.{=} [ current\_message
 {\EXCEPT} {\bang} [ self ] \.{=}\@w{none} ]}%
 \@x{\@s{100.87} \.{\land} \.{\land} stack \.{'} \.{=} [ stack {\EXCEPT}
 {\bang} [ self ] \.{=} {\langle} [ procedure
 \.{\mapsto}\@s{4.1}\@w{add\_to\_queue} ,\,}%
\@x{\@s{276.18} pc\@s{31.36} \.{\mapsto}\@s{4.10}\@w{Read} ,\,}%
\@x{\@s{276.18} val\@s{27.82} \.{\mapsto}\@s{4.1} val [ self ] ] {\rangle}}%
\@x{\@s{273.61} \.{\circ} stack [ self ] ]}%
 \@x{\@s{111.98} \.{\land} val \.{'} \.{=} [ val {\EXCEPT} {\bang} [ self ]
 \.{=} self ]}%
 \@x{\@s{100.87} \.{\land} pc \.{'} \.{=} [ pc {\EXCEPT} {\bang} [ self ]
 \.{=}\@w{Add} ]}%
\@x{\@s{100.87} \.{\land} queue \.{'} \.{=} queue}%
\@pvspace{8.0pt}%
 \@x{ reader ( self ) \.{\defeq} Read ( self ) \.{\lor} NotifyFailure ( self
 )}%
\@pvspace{8.0pt}%
\@x{ Next \.{\defeq} writer}%
 \@x{\@s{52.13} \.{\lor} ( \E\, self \.{\in} ProcSet \.{:} add\_to\_queue (
 self ) )}%
 \@x{\@s{52.13} \.{\lor} ( \E\, self \.{\in} \{\@w{r1} ,\,\@w{r2} \} \.{:}
 reader ( self ) )}%
\@pvspace{8.0pt}%
...
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Last modified Sat \ensuremath{Aug} 27
 21:45:47 \ensuremath{CST} 2022 by \ensuremath{wengjialin
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created Sat \ensuremath{Aug} 27 18:19:37
 \ensuremath{CST} 2022 by \ensuremath{wengjialin
}%
\end{cpar}%
\end{lcom}%
\end{document}
