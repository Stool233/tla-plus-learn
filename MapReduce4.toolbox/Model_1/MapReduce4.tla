...
begin
    Schedule:
        with worker_order = PT!OrderSet(Workers) do
            ...
            if Cardinality({w \in ActiveWorkers: Len(queue[w]) = 0}) > 0 then
            
                with w \in {w \in ActiveWorkers: Len(queue[w]) = 0} do
                    status[w] := "inactive";
                end with;
...
\* BEGIN TRANSLATION (chksum(pcal) = "99b6e946" /\ chksum(tla) = "8c490f83")
...
Schedule == /\ pc[Reducer] = "Schedule"
            /\ LET worker_order == PT!OrderSet(Workers) IN
                 ...
                 /\ assignments' = queue'
                 /\ IF Cardinality({w \in ActiveWorkers: Len(queue'[w]) = 0}) > 0
                       THEN /\ \E w \in {w \in ActiveWorkers: Len(queue'[w]) = 0}:
...
\* Last modified Sat Nov 26 20:16:42 CST 2022 by wengjialin
