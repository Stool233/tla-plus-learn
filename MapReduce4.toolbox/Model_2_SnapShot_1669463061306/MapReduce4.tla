...
begin
    Schedule:
        with worker_order = PT!OrderSet(Workers) do
            ...
            with ww \in {ww \in ActiveWorkers: Len(queue[ww]) = 0} do
                status[ww] := "inactive";
            end with;
...
\* BEGIN TRANSLATION (chksum(pcal) = "65f62d69" /\ chksum(tla) = "a27d8322")
...
Schedule == /\ pc[Reducer] = "Schedule"
            /\ LET worker_order == PT!OrderSet(Workers) IN
                 ...
                 /\ assignments' = queue'
                 /\ \E ww \in {ww \in ActiveWorkers: Len(queue'[ww]) = 0}:
                      status' = [status EXCEPT ![ww] = "inactive"]
...
\* Last modified Sat Nov 26 19:44:07 CST 2022 by wengjialin
