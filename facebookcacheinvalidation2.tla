...
TypeOk == 
    ...
    \* 保证下有序
    /\ invalidationQueue \in Seq(InvalidationMessage)
    /\ counter \in Nat
...
DatabaseUpdate(k) == 
    ...
    /\ database' = [database EXCEPT 
                    ![k] = updatedVersion]
    /\ invalidationQueue' = Append(invalidationQueue,
...
UpdateFromInvalidationMessage == 
    /\ invalidationQueue /= << >>
    /\ LET message == Head(invalidationQueue) IN 
        ...
        /\ invalidationQueue' = Tail(invalidationQueue)
