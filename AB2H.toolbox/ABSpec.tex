...
\@x{}\moduleLeftDash\@xx{ {\MODULE} ABSpec}\moduleRightDash\@xx{}%
\@x{ {\EXTENDS} Integers}%
\@pvspace{8.0pt}%
\@x{ {\CONSTANT} Data\@s{4.1}}%
\@y{\@s{0}%
 The set of all possible data values.
}%
\@xx{}%
\@pvspace{8.0pt}%
\@x{ {\VARIABLES} AVar ,\,\@s{8.2}}%
\@y{\@s{0}%
 The last \ensuremath{{\langle}value,\, bit{\rangle}} pair A decided to send.
}%
\@xx{}%
\@x{\@s{51.42} BVar\@s{13.68}}%
\@y{\@s{0}%
 The last \ensuremath{{\langle}value,\, bit{\rangle}} pair \ensuremath{B}
 received.
}%
\@xx{}%
\@pvspace{8.0pt}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 Type correctness means that \ensuremath{AVar} and \ensuremath{BVar} are
 tuples \ensuremath{{\langle}d,\, i{\rangle}} where
 \ensuremath{d \.{\in} Data} and \ensuremath{i \.{\in} \{0,\, 1\}}.
\end{cpar}%
\end{lcom}%
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 It\mbox{'}s useful to define \ensuremath{vars} to be the tuple of all
 variables, for
 example so we can write [\ensuremath{Next]\_vars} instead of
 [\ensuremath{Next]\_{\langle} \.{\dots} {\rangle}
}%
\end{cpar}%
\end{lcom}%
\@x{ vars \.{\defeq} {\langle} AVar ,\, BVar {\rangle}}%
\@pvspace{16.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 Initially \ensuremath{AVar} can equal \ensuremath{{\langle}d,\, 1{\rangle}}
 for any \ensuremath{Data} value \ensuremath{d}, and \ensuremath{BVar} equals
 \ensuremath{AVar}.
\end{cpar}%
\end{lcom}%
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 When \ensuremath{AVar \.{=} BVar}, the sender can ``send'' an arbitrary data
 \ensuremath{d} item by
 setting \ensuremath{AVar[1]} to \ensuremath{d} and complementing
 \ensuremath{AVar[2]}. It then waits until
 the receiver ``receives'' the message by setting \ensuremath{BVar} to
 \ensuremath{AVar} before it
 can send its next message. Sending is described by action A and
 receiving by action \ensuremath{B}.
\end{cpar}%
\end{lcom}%
\@x{ A\@s{0.63} \.{\defeq} \.{\land} AVar \.{=} BVar}%
 \@x{\@s{26.95} \.{\land} \E\, d\@s{10.11} \.{\in} Data \.{:} AVar \.{'} \.{=}
 {\langle} d ,\, 1 \.{-} AVar [ 2 ] {\rangle}}%
\@x{\@s{26.95} \.{\land} BVar \.{'} \.{=} BVar}%
\@pvspace{8.0pt}%
\@x{ B \.{\defeq} \.{\land} AVar \.{\neq} BVar}%
\@x{\@s{26.95} \.{\land} BVar \.{'} \.{=} AVar}%
\@x{\@s{26.95} \.{\land} AVar \.{'}\@s{0.62} \.{=} AVar}%
\@pvspace{8.0pt}%
\@x{ Next \.{\defeq} A \.{\lor} B}%
\@pvspace{8.0pt}%
\@x{ Spec\@s{1.46} \.{\defeq} Init \.{\land} {\Box} [ Next ]_{ vars}}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
For understanding the spec, it\mbox{'}s useful to define formulas that should
 be invariants and check that they are invariant. The following
 invariant \ensuremath{Inv} asserts that, if \ensuremath{AVar} and
 \ensuremath{BVar} have equal second
 components, then they are equal (which by the invariance of
 \ensuremath{TypeOK
} implies that they have equal first components).
\end{cpar}%
\end{lcom}%
 \@x{ Inv \.{\defeq} ( AVar [ 2 ] \.{=} BVar [ 2 ] ) \.{\implies} ( AVar \.{=}
 BVar )}%
\@x{}\midbar\@xx{}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{FairSpec} is \ensuremath{Spec} with the addition requirement that
 it keeps taking
 steps.
\end{cpar}%
\end{lcom}%
\@x{ FairSpec \.{\defeq} Spec \.{\land} {\WF}_{ vars} ( Next )}%
\@x{}\bottombar\@xx{}%
\setboolean{shading}{false}
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 18:23:37 \ensuremath{CST} 2022 by \ensuremath{wengjialin
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Last modified \ensuremath{Wed}
 \ensuremath{Oct} 18 04:07:37 \ensuremath{PDT} 2017 by \ensuremath{lamport
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Fri} \ensuremath{Sep} 04
 07:08:22 \ensuremath{PDT} 2015 by \ensuremath{lamport
}%
\end{cpar}%
\end{lcom}%
\end{document}
