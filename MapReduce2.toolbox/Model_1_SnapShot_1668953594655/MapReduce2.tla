    ...
    ReduceResult:
        while \E w \in Workers: ~consumed[w] do
            ...
            or
                \* Reassign
                with 
                    from_worker \in {w \in UnfairWorkers: ~consumed[w] /\
                        result[w] = NULL},
...
\* BEGIN TRANSLATION (chksum(pcal) = "483a2353" /\ chksum(tla) = "2ef657cb")
...
ReduceResult == /\ pc[Reducer] = "ReduceResult"
                /\ IF \E w \in Workers: ~consumed[w]
                      THEN /\ \/ /\ \E w \in {w \in Workers: ~consumed[w] /\ result[w] /= NULL}:
                              ...
                              \/ /\ \E from_worker \in             {w \in UnfairWorkers: ~consumed[w] /\
                                                       result[w] = NULL}:
...
\* Last modified Sun Nov 20 22:10:15 CST 2022 by wengjialin
