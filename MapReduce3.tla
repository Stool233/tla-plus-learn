----------------------------- MODULE MapReduce3 -----------------------------
(*------------------------------------------------------------------------*)
(* A final spec that works even if the recovery mechanism partially fails *)
(* we don’t have a way of distinguishing failing nodes from passing ones. *)
(*Before, the server could move the queue of an unfair worker to a fair one. 
Now, the server can still move the assignments of an unfair worker but does not know which ones are fair. 
Instead, it must decide which worker to pick. 
We will continue to assume the system never reassigns away from a fair worker, 
as that worker always responds to the heartbeat.
*)
(*------------------------------------------------------------------------*)
...
\* BEGIN TRANSLATION (chksum(pcal) = "4e4f8cf0" /\ chksum(tla) = "9254830e")
...
\* Last modified Sat Nov 26 20:31:46 CST 2022 by wengjialin
\* Created Mon Nov 21 01:01:55 CST 2022 by wengjialin
