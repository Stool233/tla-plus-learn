...
\@x{}\moduleLeftDash\@xx{ {\MODULE} TwoPhase}\moduleRightDash\@xx{}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 This specification is discussed in ``Two-Phase \ensuremath{Commit}'', Lecture
 6 of the
 TLA+ Video Course. It describes the Two-Phase \ensuremath{Commit} protocol,
 in
 which a transaction manager (\ensuremath{TM}) coordinates the resource
 managers
 (\ensuremath{RMs}) to implement the Transaction \ensuremath{Commit}
 specification of module
 \ensuremath{TCommit}. In this specification, \ensuremath{RMs} spontaneously
 issue \ensuremath{Prepared
 } messages. We ignore the \ensuremath{Prepare} messages that the
 \ensuremath{TM} can send to the
 \ensuremath{RMs}.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 For simplicity, we also eliminate \ensuremath{Abort} messages sent by an
 \ensuremath{RM} when it
 decides to abort. Such a message would cause the \ensuremath{TM} to abort the
 transaction, an event represented here by the \ensuremath{TM} spontaneously
 deciding
 to abort.
\end{cpar}%
\end{lcom}%
...
\@pvspace{8.0pt}%
\@x{ {\VARIABLES}}%
\@x{\@s{8.2} rmState ,\,\@s{27.41}}%
\@y{\@s{0}%
 \ensuremath{rmState[r]} is the state of resource manager \ensuremath{r}.
}%
\@xx{}%
\@x{\@s{8.2} tmState ,\,\@s{28.30}}%
\@y{\@s{0}%
 The state of the transaction manager.
}%
\@xx{}%
\@x{\@s{8.2} tmPrepared ,\,\@s{12.30}}%
\@y{\@s{0}%
 The set of \ensuremath{RMs} from which the \ensuremath{TM} has received
 \ensuremath{\@w{Prepared}
}}%
\@xx{}%
\@x{\@s{76.84}}%
\@y{\@s{0}%
...
In the protocol, processes communicate with one another by sending
 ...
 variable \ensuremath{msgs} whose value is the set of all messages that have
 been
 sent. A message is sent by adding it to the set \ensuremath{msgs}. An action
 that, in an implementation, would be enabled by the receipt of a
 certain message is here enabled by the presence of that message in
 \ensuremath{msgs}. For simplicity, messages are never removed from
 \ensuremath{msgs}. This
 ...
 particular protocol, that\mbox{'}s not a problem.
\end{cpar}%
\end{lcom}%
\@pvspace{8.0pt}%
\@x{ Messages \.{\defeq}}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The set of all possible messages. Messages of type \ensuremath{\@w{Prepared}}
 are
 sent from the \ensuremath{RM} indicated by the message\mbox{'}s
 \ensuremath{rm} field to the \ensuremath{TM}.
 Messages of type \ensuremath{\@w{Commit}} and \ensuremath{\@w{Abort}} are
 broadcast by the \ensuremath{TM}, to be
 received by all \ensuremath{RMs}. The set \ensuremath{msgs} contains just a
 single copy of
 such a message.
\end{cpar}%
\end{lcom}%
 \@x{\@s{8.2} [ type \.{:} \{\@w{Prepared} \} ,\, rm \.{:} RM ]\@s{4.1}
 \.{\cup}\@s{4.1} [ type \.{:} \{\@w{Commit} ,\,\@w{Abort} \} ]}%
\@pvspace{8.0pt}%
\@x{ TPTypeOK \.{\defeq}}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\@x{\@s{8.2} \.{\land} tmState\@s{0.89} \.{\in} \{\@w{init} ,\,\@w{done} \}}%
\@x{\@s{8.2} \.{\land} tmPrepared \.{\subseteq} RM}%
...
\@x{\@s{8.2} \.{\land} tmState\@s{0.89} \.{=}\@w{init}}%
\@x{\@s{8.2} \.{\land} tmPrepared\@s{8.2} \.{=} \{ \}}%
\@x{\@s{8.2} \.{\land} msgs \.{=} \{ \}}%
\@x{}\midbar\@xx{}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
We now define the actions that may be performed by the processes, first
 the \ensuremath{TM}\mbox{'}s actions, then the \ensuremath{RMs\.{'}} actions.
\end{cpar}%
\end{lcom}%
\@x{ TMRcvPrepared ( r ) \.{\defeq}}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The \ensuremath{TM} receives a \ensuremath{\@w{Prepared}} message from
 resource manager \ensuremath{r}. We
 could add the additional enabling condition \ensuremath{r \.{\notin}
 tmPrepared},
 which disables the action if the \ensuremath{TM} has already received this
 message. But there is no need, because in that case the action has
 no effect; it leaves the state unchanged.
\end{cpar}%
\end{lcom}%
\@x{\@s{8.2} \.{\land} tmState \.{=}\@w{init}}%
 \@x{\@s{8.2} \.{\land} [ type \.{\mapsto}\@w{Prepared} ,\, rm \.{\mapsto} r ]
 \.{\in} msgs}%
\@x{\@s{8.2} \.{\land} tmPrepared \.{'} \.{=} tmPrepared \.{\cup} \{ r \}}%
 \@x{\@s{8.2} \.{\land} {\UNCHANGED} {\langle} rmState ,\, tmState ,\, msgs
 {\rangle}}%
\@pvspace{8.0pt}%
\@x{ TMCommit \.{\defeq}}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The \ensuremath{TM} commits the transaction; enabled iff the \ensuremath{TM}
 is in its initial
 state and every \ensuremath{RM} has sent a \ensuremath{\@w{Prepared}}
 message.
\end{cpar}%
\end{lcom}%
\@x{\@s{8.2} \.{\land} tmState \.{=}\@w{init}}%
\@x{\@s{8.2} \.{\land} tmPrepared \.{=} RM}%
\@x{\@s{8.2} \.{\land} tmState \.{'} \.{=}\@w{done}}%
 \@x{\@s{8.2} \.{\land} msgs \.{'} \.{=} msgs \.{\cup} \{ [ type
 \.{\mapsto}\@w{Commit} ] \}}%
 \@x{\@s{8.2} \.{\land} {\UNCHANGED} {\langle} rmState ,\, tmPrepared
 {\rangle}}%
\@pvspace{8.0pt}%
\@x{ TMAbort \.{\defeq}}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
The \ensuremath{TM} spontaneously aborts the transaction.
\end{cpar}%
\end{lcom}%
\@x{\@s{8.2} \.{\land} tmState \.{=}\@w{init}}%
\@x{\@s{8.2} \.{\land} tmState \.{'} \.{=}\@w{done}}%
...
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Resource manager \ensuremath{r} prepares.
\end{cpar}%
\end{lcom}%
\@x{\@s{8.2} \.{\land} rmState [ r ] \.{=}\@w{working}}%
 ...
 \.{=}\@w{prepared} ]}%
 \@x{\@s{8.2} \.{\land} msgs \.{'} \.{=} msgs \.{\cup} \{ [ type
 \.{\mapsto}\@w{Prepared} ,\, rm \.{\mapsto} r ] \}}%
 \@x{\@s{8.2} \.{\land} {\UNCHANGED} {\langle} tmState ,\, tmPrepared
 {\rangle}}%
\@pvspace{8.0pt}%
\@x{ RMChooseToAbort ( r ) \.{\defeq}}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 Resource manager \ensuremath{r} spontaneously decides to abort. As noted
 above, \ensuremath{r
...
\@x{\@s{8.2} \.{\land} rmState [ r ] \.{=}\@w{working}}%
 \@x{\@s{8.2} \.{\land} rmState \.{'} \.{=} [ rmState {\EXCEPT} {\bang} [ r ]
 \.{=}\@w{aborted} ]}%
 \@x{\@s{8.2} \.{\land} {\UNCHANGED} {\langle} tmState ,\, tmPrepared ,\, msgs
 {\rangle}}%
\@pvspace{8.0pt}%
\@x{ RMRcvCommitMsg ( r ) \.{\defeq}}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Resource manager \ensuremath{r} is told by the \ensuremath{TM} to commit.
\end{cpar}%
\end{lcom}%
...
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Resource manager \ensuremath{r} is told by the \ensuremath{TM} to abort.
\end{cpar}%
\end{lcom}%
...
\@pvspace{8.0pt}%
\@x{ TPNext \.{\defeq}}%
\@x{\@s{8.2} \.{\lor} TMCommit \.{\lor} TMAbort}%
\@x{\@s{8.2} \.{\lor} \E\, r \.{\in} RM \.{:}}%
 \@x{\@s{27.51} TMRcvPrepared ( r ) \.{\lor} RMPrepare ( r ) \.{\lor}
 RMChooseToAbort ( r )}%
\@x{\@s{35.71} \.{\lor} RMRcvCommitMsg ( r ) \.{\lor} RMRcvAbortMsg ( r )}%
\@x{}\midbar\@xx{}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{lcom}%
\@pvspace{8.0pt}%
 \@x{ TPSpec \.{\defeq} TPInit \.{\land} {\Box} [ TPNext ]_{ {\langle} rmState
 ,\, tmState ,\, tmPrepared ,\, msgs {\rangle}}}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
The complete spec of the Two-Phase \ensuremath{Commit} protocol.
\end{cpar}%
\end{lcom}%
\@pvspace{8.0pt}%
\@x{ {\THEOREM} TPSpec \.{\implies} {\Box} TPTypeOK}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 This theorem asserts that the type-correctness predicate
 \ensuremath{TPTypeOK} is
...
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
We now assert that the Two-Phase \ensuremath{Commit} protocol implements the
 Transaction \ensuremath{Commit} protocol of module \ensuremath{TCommit}. The
 following statement
 imports all the definitions from module \ensuremath{TCommit} into the current
 module.
\end{cpar}%
\end{lcom}%
\@x{ {\INSTANCE} TCommit}%
\@pvspace{8.0pt}%
\@x{ {\THEOREM} TPSpec \.{\implies} TCSpec}%
\begin{lcom}{8.2}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 This theorem asserts that the specification \ensuremath{TPSpec} of the
 Two-Phase
 Commit protocol implements the specification \ensuremath{TCSpec} of the
 Transaction \ensuremath{Commit} protocol.
\end{cpar}%
\end{lcom}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The two theorems in this module have been checked with \ensuremath{TLC} for
 six
 \ensuremath{RMs}, a configuration with 50816 reachable states, in a little
 over a
 minute on a 1 \ensuremath{GHz} \ensuremath{PC}.
\end{cpar}%
\end{lcom}%
\@pvspace{8.0pt}%
\@x{}\bottombar\@xx{}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 \ensuremath{Jun} 08 21:02:57 \ensuremath{CST} 2022 by \ensuremath{wengjialin
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Wed} \ensuremath{Jun} 08
 00:02:31 \ensuremath{CST} 2022 by \ensuremath{wengjialin
}%
\end{cpar}%
\end{lcom}%
\end{document}
